...
blogs = sample(c(0,1,2,3,4), size=n, replace=T, prob = c(1, .5, .2, .1, .1)) %>% as.character
...
paranormal = data.frame(conviction = conviction %>% round(),
                        fear=fear %>% round(), time=time %>% round(), kidnapped=kidnapped, experiences.type=experiences.type,
                        income=income, blogs=blogs, age=age %>% round(), gender, political)
write.csv(paranormal, "data/paranormal.csv", row.names=F)
usethis::use_data(paranormal, overwrite = TRUE)## attractiveness data
set.seed(2323)
n = 207
stats_skills = rnorm(n)
group = sample(c("control", "mo money", "wealth building"), size=n, replace=T)
...
paranormal = data.frame(conviction = conviction,
                        ...
                        income=income, blogs=blogs, age=age, gender, political)
write.csv(paranormal, "data/paranormal.csv", row.names=F)
...
#'   \item{blogs}{Number of blogs they follow on the paranormal}
...
d = d_final
d$ptsd = round((5*(d$minutes.fighting))/(5 + (d$minutes.fighting)) + d$ptsd, digits=1)
d_final = d
