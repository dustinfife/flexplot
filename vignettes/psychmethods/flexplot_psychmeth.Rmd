...
plot = ggplot(data = exercise_data, aes(x=therapy.type, y=weight.loss)) + # x/y axis layer
  geom_jitter(width = .2, alpha = .4) + # point layer
  stat_summary(fun.y = 'mean', geom = 'point', 
        size = 3, position = position_dodge(width = .2)) + # summary point layer
  stat_summary(geom = 'errorbar', fun.ymin = function(z){mean(z)-1.96*sd(z)}, 
        fun.ymax=function(z) {mean(z)+1.96*sd(z)}, 
        size = 1.25, width = .2, position = position_dodge(width = .2))  # "errorbar" layer
```
