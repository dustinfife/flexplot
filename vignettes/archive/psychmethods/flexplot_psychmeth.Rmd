...
keywords: flexplot, statistical assumptions, statistical modeling, replication crisis, graphics, exploratory data analysis, graphical data analysis
...
knitr::opts_knit$set(root.dir = '~/Documents/RPackages/flexplot/vignettes/psychmethods')
...
```{r, ses, echo=FALSE, fig.cap = "Simulated relationship between `Treatment`, `Depression`, and `SES`. The graph makes it clear there's an interaction effect, which would be masked if one simply computed a main effects model.", message=FALSE, warning=FALSE}
require(ggplot2)
require(flexplot)
data(exercise_data)
ses = rnorm(1000)
...
```{r, message=FALSE, echo=TRUE, warning=FALSE, options(prompt=" ", continue=" "), size="small"}
...
```{r, message=FALSE, echo=TRUE, warning=FALSE, size="small"}
...
The base `plot()` function in `R` follows similar conventions as `flexplot` (i.e., users can specify an equation, such as `plot(y  ~  x)`), though `flexplot` is more intelligent in its choice of displays.[^int] Also, `plot()` only allows the user to visualize one variable at a time. Another function, `coplot()`, allows some multivariate visualizations, yet it is limited in the types of data allowed and, like `plot()` is not flexible in the types of visualization decisions it makes. The `flexplot` package, on the other hand, offers great flexibility and automates much of the decision-making. 

[^int]: R's `plot()` function is actually a S3 method, so it is actually offers a great deal of flexibility for visualizing all sorts of R objects, much like flexplot's `visualize()` method. However, its flexibility is limited to package developers who must create a class to leverage the `plot()` method. The point I'm making here is not about the various S3 methods, but about R's generic `plot()` function. 
...
```{r, bar, fig.cap = "A histogram (left) and barchart (right) produced within `flexplot` in R.", message=FALSE, echo=TRUE, warning=FALSE, size="small"}
...
```{r, chunkit, fig.cap = "Two displays of the same data. In the left image, categories are sorted alphabetically (which is the default of `ggplot2`). In `flexplot` the categories are sorted by sample size, which enables better chunking of information. \\label{fig:chunkit}", message=FALSE, echo=FALSE, size="small"}
a = flexplot(party~1, 
...
```{r, raw, message=FALSE, echo=TRUE, warning=FALSE, fig.cap = "Violin plots with 15,000 versus 15 datapoints. The outlines look the same in the left image, but the right image overlays the raw data, which makes the differing sample sizes much more apparent. \\label{fig:raw}", fig.width = 6, fig.height = 3, out.width = "90%", fig.align = 'center', size="small"}
...
```{r, jitterit, message=FALSE, echo=TRUE, warning=FALSE, fig.cap = "Dot plots with the interquartile range and no jittering (left), beeswarm plot with mean + standard errors and jittering on $X$ and $Y$ (middle), and beeswarm plot with mean + standard deviation with jittering on only $X$ (right).", fig.width = 6, fig.height = 2.5, out.width = "90%", fig.align = 'center', size="small"}
...
```{r, scatter, message=FALSE, echo=TRUE, warning=FALSE, fig.cap = "Scatterplot with different options of fit: loess (default), lm (regression), and quadratic Also, the data in the far right plot has been jittered. ", fig.width = 6, fig.height = 2.5, fig.align = 'center', size="small"}
...
```{r, logistic, message=FALSE, echo=TRUE, warning=FALSE, fig.cap = "Example of a logistic plot in the `flexplot` package.\\label{fig:logistic}", fig.width = 3, fig.height = 3, size="small"}
...
```{r, association, message=FALSE, echo=TRUE, warning=FALSE, fig.cap = "Example of an association plot for categorical predictor/categorical outcome.\\label{fig:association}" ,fig.width = 3.5, fig.height = 2.5, size="small"}
...
```{r, plant, message=FALSE, echo=TRUE, warning=FALSE,fig.width = 4, fig.height = 3, fig.cap = "A plot of repeated measures data.\\label{fig:plant}", size="small"}
data("plant_growth")
...
```{r, sample, message=FALSE, echo=TRUE, warning=FALSE, fig.cap = "Four graphics showing different ways to handle overlapping datapoints. The top-left image does nothing. The top-right omits raw data. The bottom-left reduces the opacity of the points. The bottom-right samples datapoints. \\label{fig:sample}", size="small"}
data("nsduh")
...
```{r, avp, message=FALSE, echo=TRUE, warning=FALSE, fig.width = 5, fig.height = 3, fig.cap = "An Added Variable Plot (AVP) showing the relationship between `therapy.type` and `weight.loss`, after controlling for `motivation`. \\label{fig:avp}", size="small"}
...
```{r, symbols, message=FALSE, echo=TRUE, warning=FALSE, fig.width = 8, fig.height = 4, fig.cap = "Two multivariate graphs illustrating how the second slot in a `flexplot` formula controls the graph. The left image demonstrates what happens to the second slot variable ($X2$) when a numeric predictor is on the X-axis, while the right image demonstrates what happens to the second slot variable when a categorical predictor is on the X-axis.", size="small"}
...
```{r, panels, message=FALSE, echo=FALSE, warning=FALSE, size="small", fig.cap = "A multivariate plot where `therapy.type` and `gender` are now shown in panels. \\label{fig:panels}", fig.width = 8, fig.height = 8}
...
```{r, panels2, fig.align = "center", out.width = "100%",fig.height = 6, fig.width = 8,  fig.cap = "Plots reflecting choices of different break points (left-most plot), labels (right plot), and bins (bottom plot). \\label{fig:panels2}",message=FALSE, echo=FALSE, warning=FALSE, size="small"}
...
It is easy for multivariate graphs to become unnecessarily complicated. For example, the top image in Figure \@ref(fig:threeway) shows a graphic where all four `flexplot` slots are occupied. This is very difficult to interpret. To simplify things, we could utilize several strategies. First, we can specify two bins instead of the default three. We can also remove confidence intervals, reduce the opacity of the data, and plot regression lines.[^foot2] Also, I generally try to have only two levels for the variable in the second slot (in this case, Male versus Female). Finally, we could add ghost lines. The bottom image in Figure \@ref(fig:threeway) utilizes all these strategies and simplifies the visual interpretation immensely.
...
```{r, ancova, message=FALSE, echo=TRUE, warning=FALSE, fig.cap = "Demonstration of the `visualize` function on a `lm` object. The top row shows a representation of the statistical model. The bottom row shows diagnostic plots.  \\label{fig:ancova}", fig.width=8, fig.height=8, size="small"}
...
```{r, ancova2, message=FALSE, echo=TRUE, warning=FALSE, fig.cap = "Demonstration of the `visualize` function on a `lm` object, but with `flexplot` arguments controlling the output (as well as suppressing residuals).  \\label{fig:ancova2}", size="small"}
...
visualize(model, formula = weight.loss ~ motivation | therapy.type, 
          ghost.line = "red", method = "lm", plot = "model")
```
...
```{r, mixed, message=FALSE, echo=TRUE, warning=FALSE, fig.cap = "Demonstration of the `visualize` function for a mixed model. In this graphic, each thin line represents the fit of a particular school. \\label{fig:mixed}", size="small"}
require(lme4)
data(math)
...
```{r, compare, message=FALSE, echo=TRUE, warning=FALSE, fig.cap = "Demonstration of the `compare.fits()` function, comparing a main effects and an interaction model. Ghost lines have been added from the `cog` condition for easier comparison across panels. \\label{fig:compare}", fig.width = 8, fig.height = 4.5, size="small"}
