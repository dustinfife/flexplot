flexplot_modify_prediction = function(prediction, formula=NULL,
  ...
  number_of_models = length(num.models)
...
flexplot_generate_prediction_lines = function(data, plot, axis){
  ...
  if (!is.numeric(data[[axis[1]]])) {
    plot$pred.line = 'geom_point(data=prediction, aes(y=prediction, color=model), position=position_dodge(width=.2)) + 
  ...
  if (!is.na(axis[2])){
    plot$pred.line = 'geom_line(data= prediction, aes_string(linetype=axis[2], y="prediction", colour=axis[2]), size=1)' 				
    plot$fitted = "xxxx"
    return(plot)
  }  
