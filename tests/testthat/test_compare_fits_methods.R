...
test_that("get_fitted works with various model types", {
  ...
  get_fitted(model = lme4::glmer(a~x +y + (y | id), data=small_mixed, family=binomial), pred.values = small_mixed)
  polyn = (weight.loss~rewards,               data=exercise_data %>% mutate(weight.loss = factor(weight.loss, ordered=T)))
...
test_that("compare.fits with rlm and polr", {
  ...
  compare_fits(formula=weight.loss~rewards, data=d, model1=mod, model2=mod2)
